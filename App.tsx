...
const App: React.FC = () => {
  const [screen, setScreen] = useState<Types.Screen>('MENU');
  ...
  const gameLoop = useCallback((time: number) => {
    ...
    setGameState(prev => {
      ...
      if (player.actionState === 'ATTACK_KNIFE' || player.actionState === 'ATTACK_GUN') {
        player.actionTimer = (player.actionTimer || 0) - delta;
        if (player.actionTimer <= 0) {
          player.actionState = 'IDLE';
          player.actionTimer = 0;
        }
      } else if (player.actionState === 'RELOADING') {
        player.actionTimer = (player.actionTimer || 0) - delta;
        if (player.actionTimer <= 0) {
          // RELOAD COMPLETE
          player.actionState = 'IDLE';
          ...
          const gun = player.inventory.find(i => i.type === Types.ItemType.GUN);
          const ammo = player.inventory.find(i => i.type === Types.ItemType.AMMO);

          if (gun && ammo) {
            gun.count = 12;
            player.inventory = player.inventory.filter(i => i.id !== ammo.id);
            setGameState(prev => prev ? { ...prev, message: '换弹完成', messageTimeout: 1.0 } : null);
          } else {
            setGameState(prev => prev ? { ...prev, message: '换弹失败：资源缺失', messageTimeout: 1.0 } : null);
          }
        }
      }
    ...
    entities.forEach(e => {
      ...
      renderList.push({
        ...
        draw: () => {
          ...
          const isMonster = e.type === Types.EntityType.MONSTER;

          if (e.type === Types.EntityType.CHEST) {
            const size = 32;
            const img = e.data.isOpen ? chestOpenImgRef.current : chestClosedImgRef.current;
            if (img) {
              ctx.drawImage(img, cx - size / 2, cy - size / 2 - 10, size, size);
            } else {
              ctx.fillStyle = e.data.isOpen ? '#3a2601' : '#a16207';
              ctx.fillRect(cx - 14, cy - 14, 28, 28);
            }
          } else if (isMonster || isBoss) {
            const size = isBoss ? 80 : 50; // Larger size for Boss

            ctx.save();
            ctx.translate(cx, cy);

            // Hit Flash & Rage Effect
            const isRage = isBoss && e.health! <= e.data.maxHealth! * 0.5;
            if (e.data.hitFlash > 0) {
              ctx.globalAlpha = 0.7;
              ctx.fillStyle = '#ef4444';
              ctx.beginPath(); ctx.arc(0, -15, size / 2, 0, Math.PI * 2); ctx.fill();
              ctx.globalAlpha = 1.0;
            }
            if (isRage) {
            ...
            if (isBoss) {
              ...
              // Simple Boss HP Bar
              const hpPct = e.health! / e.data.maxHealth!;
              ctx.fillStyle = 'black';
              ctx.fillRect(-40, -size - 20, 80, 8);
              ctx.fillStyle = isRage ? '#ef4444' : '#fbbf24';
              ctx.fillRect(-40, -size - 20, 80 * hpPct, 8);
            } else {
              if (monsterImgRef.current) {
                ctx.drawImage(monsterImgRef.current, -size / 2 + shake, -size / 2 - 15 + bounce, size, size);
              } else {
                ctx.font = `${size}px serif`;
              ...
              // Monster HP Bar
              ctx.fillStyle = 'red';
              ctx.fillRect(-20, -size + 10, 40 * (e.health! / 75), 4);
            }
            ctx.restore();
